...
```
Bash: echo "▶ [PR] Início: $(date '+%H:%M:%S')" && date +%s > /tmp/squad_agent_start && git add -A && git commit -m "US-NNN: artefatos da squad" --allow-empty 2>/dev/null; git push -u origin us-NNN --force 2>&1 && gh pr create --base main --head us-NNN --title "US-NNN: <titulo>" --body "Implementação da US-NNN." && START=$(cat /tmp/squad_agent_start); END=$(date +%s); D=$((END-START)); printf "✅ [PR] Fim: %s | ⏱️ %d:%02d\n" "$(date '+%H:%M:%S')" "$((D/60))" "$((D%60))"
```
